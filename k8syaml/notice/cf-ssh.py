...
cf_redirect_host        = "home.yxs.one"
cf_direct_host          = "home-d.yxs.one"
...
def main():
    ...
    content = f"-- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("ssh service update",title,content)
    if error:
        print(error)
    print("send ok!")

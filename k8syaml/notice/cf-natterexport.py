...
cf_redirect_host        = "natter.yxs.one"
cf_direct_host          = "natter-d.yxs.one"
cf_auth_email           = "xxx@gmail.com"
cf_auth_key             = "xxxxaaa"
# 获取当前时间
...
def main():
    ...
    title = f"publicIP：[ {public_ip}:{public_port} ]"
    content = f"natter-exportservice service--- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("natterexportconfig service update",title,content)
    if error:
        print(error)
    print("send ok!")

    if not cf.send_get_request("natterexport_port"):
        print("Stopping container due to request failure.")
        # 停止容器
        os._exit(1)  # 退出时返回非零状态
...
class CloudFlareRedir:
    ...
    def send_get_request(self,targetKey):
        try:
        # 创建请求对象
            url = f"http://localhost:8080/updateValues?key={targetKey}&value={public_ip}:{public_port}&token=token4538"
    ...
    def send_message(self,title, description, content):

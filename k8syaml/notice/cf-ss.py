...
cf_redirect_host        = "ss.yxs.one"
cf_direct_host          = "ss-d.yxs.one"
...
def main():
    ...
    content = f"ss service--- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("ss service update",title,content)
    if error:
        print(error)
    print("send ok!")

    if not cf.send_get_request("ss_port"):

...
cf_redirect_host        = "qb.yxs.one"
cf_direct_host          = "qb-d.yxs.one"
...
def main():
    ...
    content = f"qb service--- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("qb service update",title,content)
    if error:
        print(error)
    print("send ok!")

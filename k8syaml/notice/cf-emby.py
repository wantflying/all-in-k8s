...
cf_redirect_host        = "emby.yxs.one"
cf_direct_host          = "emby-d.yxs.one"
...
def main():
    ...
    content = f"emby service--- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("emby service update",title,content)
    if error:
        print(error)
    print("send ok!")

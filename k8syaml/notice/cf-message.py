...
cf_redirect_host        = "message.yxs.one"
cf_direct_host          = "message-d.yxs.one"
...
def main():
    ...
    content = f"message service--- {formatted_time}：[  {public_ip}:{public_port} {cf_direct_host}:{public_port} ]"
    error = cf.send_message("message service update",title,content)
    if error:
        print(error)
    print("send ok!")

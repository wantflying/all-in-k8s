...
try:
    ...
    if len(daily_weather.get("precipitation", [])) > 1:
        ...
        # 构建输出内容
        output_description += f"**日期**: {first_day['date']} 🗓️ \n"
        output_description += f"**白天降水概率**: {first_day['probability']}% 🌧️ \n"
        output_description += f"**最高气温**: {first_day_temp['max']}°C, **最低气温**: {first_day_temp['min']}°C \n\n"

        output_description += f"**日期**: {second_day['date']} 🗓️\n"
        output_description += f"**白天降水概率**: {second_day_day_precipitation['probability']}% 🌧️\n"
        ...
        if rain_today or temp_difference > 5:
            msg_des = ''
            output_content += "⚠️ **注意事项** ⚠️\n"
            if rain_today:
                output_content += "🌧️ 未来两天可能会下雨，请注意带伞！\n"
                msg_des = f"两天内可能会下雨:{first_day['probability']}% {second_day_day_precipitation['probability']}%"

            if temp_difference > 5:
                output_content += f"🌡️ 温差超过5度，注意适时增减衣物！当前温差: {temp_difference}°C\n"
